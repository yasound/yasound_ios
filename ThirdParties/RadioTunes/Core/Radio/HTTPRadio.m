...
- (void)handlePropertyChange:(AudioFileStreamID) inAudioFileStream 
    ...
    if(inPropertyID == kAudioFileStreamProperty_ReadyToProducePackets) {
        ...
        err = AudioQueueAddPropertyListener(_playerState.queue, kAudioQueueProperty_IsRunning, QueuePropertyListenerProc, self);
        if(err) {
            DLog(@"Error: Cannot add listener to AudioQueue for property kAudioQueueProperty_IsRunning %ld", err);
            _radioError = kRadioErrorAudioQueueCreate;
            [self setState:kRadioStateError];
            return;
        }
...
- (void)handleQueuePropertyChange:(AudioQueueRef)inAudioQueue
    ...
    if(inPropertyID == kAudioQueueProperty_IsRunning) {
        if(_waitForEmptyBuffers && _playerState.playing) {
            _playerState.playing = NO;
            _playerState.paused = YES;
...
- (void)dismissQueue {
    if(_playerState.queue) {
        if(_playerState.playing) {
            if(_waitForEmptyBuffers) {
                AudioQueueStop(_playerState.queue, NO);
            } else {
