...
- (id)initWithFrame:(CGRect)frame reuseIdentifier:(NSString*)CellIdentifier event:(WallEvent*)ev height:(CGFloat)height indexPath:(NSIndexPath*)indexPath
{
  ...
  {    
    ...
    UIView* view = self.contentView;
      ...
      sheet = [[Theme theme] stylesheetForKey:@"Wall.cellLike.background" retainStylesheet:YES overwriteStylesheet:NO error:nil];
      view.backgroundColor = [UIColor colorWithPatternImage:[sheet image]];
    
    sheet = [[Theme theme] stylesheetForKey:@"Wall.cellLike.picto" retainStylesheet:YES overwriteStylesheet:NO error:nil];
    [view addSubview:[sheet makeImage]];

    sheet = [[Theme theme] stylesheetForKey:@"Wall.cellLike.message" retainStylesheet:YES overwriteStylesheet:NO error:nil];
